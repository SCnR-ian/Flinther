...
// Returns conflicts between proposed settings and future bookings/coaching/social.
  ...
  try {
      ...
      if (new_court_count < courts.length) {
        ...
        for (const r of rows) {
          conflicts.push({
            ...
            end_time: r.end_time,
            description: `${r.user_name} — ${r.court_name}`,
            reason: 'court_removed',
          })
        }
      }
    }
    ...
    if (Array.isArray(schedule)) {
      for (const day of schedule) {
        ...
        if (!day.is_active) {
          // Day closed — find all future sessions on this day
          ...
          for (const r of bRows) {
            conflicts.push({ type: 'booking', date: r.date, start_time: r.start_time, end_time: r.end_time, description: r.user_name, reason: 'day_closed' })
          }
          ...
          for (const r of cRows) {
            conflicts.push({ type: 'coaching', date: r.date, start_time: r.start_time, end_time: r.end_time, description: `${r.student_name} × ${r.coach_name}`, reason: 'day_closed' })
          }
          ...
          for (const r of sRows) {
            conflicts.push({ type: 'social', date: r.date, start_time: r.start_time, end_time: r.end_time, description: r.title, reason: 'day_closed' })
          }

        } else {
          // Day active but hours may have changed — find sessions outside new hours
          const { rows: bRows } = await pool.query(
            `SELECT DISTINCT ON (b.booking_group_id)
               b.date, MIN(b.start_time) AS start_time, MAX(b.end_time) AS end_time,
               u.name AS user_name
          ...
          for (const r of bRows) {
            conflicts.push({ type: 'booking', date: r.date, start_time: r.start_time, end_time: r.end_time, description: r.user_name, reason: 'outside_hours' })
          }
          ...
          for (const r of cRows) {
            conflicts.push({ type: 'coaching', date: r.date, start_time: r.start_time, end_time: r.end_time, description: `${r.student_name} × ${r.coach_name}`, reason: 'outside_hours' })
          }
          ...
          for (const r of sRows) {
            conflicts.push({ type: 'social', date: r.date, start_time: r.start_time, end_time: r.end_time, description: r.title, reason: 'outside_hours' })
          }
        }
      }
    }
