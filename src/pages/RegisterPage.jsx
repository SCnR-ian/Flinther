...
export default function RegisterPage() {
  ...
  const [inviteInfo, setInviteInfo]             = useState(null)  // { club_name }
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      // Coach invite — auto logged in, go to coach portal
      if (inviteToken) { navigate('/coach', { replace: true }); return }
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4" style={{ fontFamily: '"DM Sans", sans-serif' }}>
        <div className="w-full max-w-[360px] text-center">
          <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-6 text-3xl">✉️</div>
    ...
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4" style={{ fontFamily: '"DM Sans", sans-serif' }}>
      ...
      <div className="w-full max-w-[360px]">
        <div className="flex justify-center mb-6">
          <div className="w-12 h-12 rounded-xl bg-gray-900 flex items-center justify-center text-white text-xl" style={{ fontFamily: '"Kanit", sans-serif' }}>F</div>
        </div>
        ...
        {!inviteChecking && inviteInfo && (
          <div className="mb-6 px-4 py-3 bg-emerald-50 border border-emerald-100 rounded-xl text-center">
            <p className="text-xs text-emerald-600 font-medium uppercase tracking-wider mb-0.5">Coach Invite</p>
            <p className="text-sm text-gray-700">You're joining <span className="font-semibold">{inviteInfo.club_name}</span> as a coach.</p>
          </div>
        )}

        {!inviteChecking && !inviteError && (
          <>
            <div className="text-center mb-8">
              <h1 className="text-2xl text-gray-900 mb-1" style={{ fontFamily: '"Kanit", sans-serif' }}>
                {inviteInfo ? 'Create your coach account' : club ? `Join ${club.name}` : 'Create your account'}
              </h1>
              <p className="text-gray-400 text-sm">
                {inviteInfo ? 'Set up your account to get started.' : club ? 'Create an account to start booking.' : 'Sign up to launch your club on Flinther.'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-3">
              ...
              <div>
                <input name="email" type="email" placeholder="Email address" autoComplete="email" value={form.email} onChange={handleChange}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-gray-400 transition-colors" />
                {errors.email && <p className="text-xs text-red-500 mt-1">{errors.email}</p>}
              </div>
              <div>
                <input name="password" type="password" placeholder="Password (min. 8 characters)" autoComplete="new-password" value={form.password} onChange={handleChange}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-gray-400 transition-colors" />
                {errors.password && <p className="text-xs text-red-500 mt-1">{errors.password}</p>}
              </div>
              <div>
                <input name="confirmPassword" type="password" placeholder="Confirm password" autoComplete="new-password" value={form.confirmPassword} onChange={handleChange}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-gray-400 transition-colors" />
            ...
            <p className="text-center text-sm text-gray-400 mt-6">
              ...
              <Link to="/login" className="text-gray-900 underline underline-offset-2 hover:text-gray-500 transition-colors">Log in</Link>
            </p>
